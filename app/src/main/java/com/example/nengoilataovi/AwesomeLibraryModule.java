...
import com.sr25519.sign.KeyPair;
import com.sr25519.sign.ExpansionMode;
import com.sr25519.utils.HexUtils;
...
public class AwesomeLibraryModule extends ReactContextBaseJavaModule {
    ...
    public AwesomeLibraryModule() {
        super();
        Log.println(Log.INFO, "aaa", "TrustWalletCore" );
    ...
    public void xpubHd(String mnemonic , String chain, Promise promise) {
        ...
        if(chain.equals("cardano")){
            purpose = Purpose.BIP1852;
    ...
    @ReactMethod
    public String hdkey(String mnemonic , String path) {
        HDWallet wallet = new HDWallet(mnemonic, "");
        ...
        String publicKey = HexUtils.bytesToHex(publicKeyBase64);

        byte[] privateKeyBase64 = key.data();
        String privateKey = HexUtils.bytesToHex(privateKeyBase64);
...
//        return result;
    ...
    @ReactMethod
    public void cardano(String mnemonic , String path, Promise promise) {
        HDWallet wallet = new HDWallet(mnemonic, "");
        PrivateKey key = wallet.getKey(CoinType.CARDANO , path);
        byte[] publicKeyBase64 = key.getPublicKeyByType(PublicKeyType.ED25519CARDANO).data();
        byte[] privateKeyBase64 = key.data();
        String result = convertByteArrayToString(publicKeyBase64,privateKeyBase64);
    ...
    public void nacl(String mnemonic , String path, Promise promise) {
        ...
        if(path.equals("m")){
            ...
            publicKeyBase32 = keyPair.getPublicKey();
            privateKeyBase64 = keyPair.getSecretKey();
        }
        else {
            PrivateKey key = wallet.getKey(CoinType.SOLANA , path);
            publicKeyBase32 = key.getPublicKeyEd25519().data();
            byte[] privateKey32Byte = key.data();
            int length = privateKey32Byte.length + publicKeyBase32.length;
            byte[] result = new byte[length];
            System.arraycopy(privateKey32Byte, 0, result, 0, privateKey32Byte.length);
            System.arraycopy(publicKeyBase32, 0, result, privateKey32Byte.length, publicKeyBase32.length);
            privateKeyBase64 = result;
        }
        String result = convertByteArrayToString(publicKeyBase32,privateKeyBase64);
        promise.resolve(result);
    }
